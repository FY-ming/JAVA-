...
public class upLoadFile extends JDialog {
    public upLoadFile(Window owner, String name, Client appl) {
    ...
    private void onConfirm(ActionEvent e) {
        ...
        try {
            if(Objects.requireNonNull(DataProcessing.searchUser(this.name)).uploadFile(doc,this.name,timestamp,description,fileName))
            {
                ...
                application.sendData("CLIENT >>> UpLoadFile_SUCCESS");
                application.displayMessage("客户端 >>> 文件上传成功\n");
//                if(application.receiveServerMessage().equals("SERVER >>> UpLoadFile_SUCCESS"))
//                {
//                    application.displayMessage("客户端 >>> 信息上传至服务器\n");
//                }else {
//                    application.displayMessage("客户端 >>> 信息上传服务器失败\n");
//                }
                dispose(); // 关闭对话框
            }
        } catch (Exception ex) {
